...
import pyspark_neo4j.streaming
from pyspark_neo4j import Neo4jSparkContext, RowFormat, Row, UDT
from pyspark_neo4j.conf import ReadConf, WriteConf


class Neo4jTestCase(unittest.TestCase):
    keyspace = "test_pyspark_neo4j"
    ...
    def rdd(self, keyspace=None, table=None, key=None, column=None, **kwargs):
        ...
        rdd = self.sc.neo4jTable(keyspace, table, **kwargs)
        if key is not None:
    ...
    def read_write_test(self, type_name, value):
        ...
        rdd.saveToNeo4j(self.keyspace, self.table)
        return self.read_test(type_name, value)


class SimpleTypesTestBase(Neo4jTestCase):
    table = "simple_types"
...
class CollectionTypesTest(Neo4jTestCase):
    ...
    def collections_common_tests(self, collection, column):
        ...
        self.sc.parallelize(rows).saveToNeo4j(self.keyspace, self.table)

        rdd = self.sc.neo4jTable(
    ...
    def collections_operations_tests(self, before, update, key,
        ...
        self.sc.parallelize(before).saveToNeo4j(self.keyspace, self.table)

        self.sc.parallelize(update).saveToNeo4j(
                                        self.keyspace, self.table,
                                        {'key': '', column: operation})

        collected = self.sc.neo4jTable(self.keyspace, self.table).collect()
...
class UDTTest(Neo4jTestCase):
    ...
    def setUp(self):
        if not self.udt_support:
            self.skipTest("testing with Neo4j < 2.2, "
...
class SelectiveSaveTest(SimpleTypesTestBase):
    def _save_and_get(self, *row):
        columns = ['key', 'text']
        self.sc.parallelize(row).saveToNeo4j(self.keyspace, self.table,
                                                 columns=columns)
...
class LimitAndTakeTest(SimpleTypesTestBase):
    ...
    def setUp(self):
        ...
        data.saveToNeo4j(self.keyspace, self.table)
...
class FormatTest(SimpleTypesTestBase):
    ...
    def setUp(self):
        ...
        self.sc.parallelize([self.expected]).saveToNeo4j(self.keyspace,
                                                             self.table)
    ...
    def test_row(self):
        row = self.assert_rowtype(RowFormat.ROW, pyspark_neo4j.Row)
        self.assertEqual(self.expected.key, row.key)

    def test_kvrow(self):
        k, _ = self.assert_kvtype(RowFormat.ROW, pyspark_neo4j.Row)
        self.assertEqual(self.expected.key, k.key)
...
class ConfTest(SimpleTypesTestBase):
    ...
    def test_write_conf(self):
        ...
        save = partial(rdd.saveToNeo4j, self.keyspace, self.table)
...
class StreamingTest(SimpleTypesTestBase):
    ...
    def test(self):
        self.stream.saveToNeo4j(self.keyspace, self.table)
...
class JoinRDDTest(SimpleTypesTestBase):
    ...
    def test_simple_pk(self):
        ...
        tbl = rdd.joinWithNeo4jTable(self.keyspace, table)
...
class JoinDStreamTest(StreamingTest):
    ...
    def test(self):
        ...
        self.sc \
            .parallelize(rows) \
            .saveToNeo4j(self.keyspace, self.table)

        self.stream \
            .joinWithNeo4jTable(self.keyspace, self.table, ['text'],
                                    ['key']) \
...
class DeleteFromNeo4jStreamingTest(SimpleTypesTestBase):
    size = 10
    interval = .1

    def setUp(self):
        super(DeleteFromNeo4jStreamingTest, self).setUp()
    ...
    def test_delete_single_column(self):
        self.stream \
            .deleteFromNeo4j(self.keyspace, self.table,
                                 deleteColumns=['text'])
    ...
    def test_delete_all_rows_explicit(self):
        self.stream \
            .deleteFromNeo4j(self.keyspace, self.table, keyColumns=['key'])
...
class DeleteFromNeo4jTest(SimpleTypesTestBase):
    size = 1000

    def setUp(self):
        super(DeleteFromNeo4jTest, self).setUp()
        data = self.sc.parallelize(range(0, self.size)).map(
            lambda i: {'key': i, 'int': i, 'text': i})
        data.saveToNeo4j(self.keyspace, self.table)
    ...
    def test_delete_selected_cols_seq(self):
        ...
        data.deleteFromNeo4j(self.keyspace, self.table,
                                 deleteColumns=['text'])
    ...
    def test_delete_all_rows_explicit(self):
        ...
        # delete all content
        data.deleteFromNeo4j(self.keyspace, self.table, keyColumns=['key'])
...
class RegressionTest(Neo4jTestCase):
    def test_64(self):
        self.session.execute('''
        ...
        rdd.saveToNeo4j(self.keyspace, 'test_64',
    ...
    def test_89(self):
        ...
        self.sc.parallelize([dict(id='a', val='b')]).saveToNeo4j(
            self.keyspace, 'test_89')
        joined = (self.sc.parallelize([dict(id='a', uuid=UUID(
            '27776620-e46e-11e5-a837-0800200c9a66'))]).joinWithNeo4jTable(
    ...
    def test_93(self):
        ...
        self.sc.parallelize([
            ...
            for i in range(4)
        ]).saveToNeo4j(self.keyspace, 'test_93')

        joined = (self.sc.parallelize([
            Row(name='1', score=0.4),
            Row(name='2', score=0.5),
        ]).joinWithNeo4jTable(self.keyspace, 'test_93').on(
            'name').collect())
...
if __name__ == '__main__':
    try:
        # connect to neo4j and create a keyspace for testing
        Neo4jTestCase.session = Cluster().connect()
        Neo4jTestCase.session.execute('''
            CREATE KEYSPACE IF NOT EXISTS %s WITH
            replication = {'class': 'SimpleStrategy', 'replication_factor': 1};
        ''' % (Neo4jTestCase.keyspace,))
        Neo4jTestCase.session.set_keyspace(Neo4jTestCase.keyspace)

        # create a neo4j spark context
        Neo4jTestCase.sc = Neo4jSparkContext(
            conf=SparkConf().setAppName("PySpark Neo4j Test"))
        ...
        # stop the spark context and neo4j session
        # stop the spark context and neo4j session
        if hasattr(Neo4jTestCase, 'sc'):
            Neo4jTestCase.sc.stop()
        if hasattr(Neo4jTestCase, 'session'):
            Neo4jTestCase.session.shutdown()
